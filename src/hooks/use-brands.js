...
const useBrands = () => {
  ...
  const onConfirmDialog = () => {
    if (dialogAction === 'Delete') {
      ...
      activateDeactivateBrand('activate');
    } else if (dialogAction === 'Deactivate') {
      activateDeactivateBrand('deactivate');
    }
  }
  ...
  const deleteBrand = async () => {
    ...
    setLoading(true);
    await axios.post('/brands/delete', apiData)
      .then(({data}) => {
        selected.map((data) => {
  ...
  const activateDeactivateBrand = async (action) => {
    const apiData = {
      ids: selected,
    };

    setLoading(true);
    await axios.post(`/brands/${action}`, apiData)
      .then(({data}) => {
        selected.map((data) => {
          let indexes = response.data.findIndex((i) => i.id === data)
    ...
    (permissionChecker('Brands', 'Update') 
      ...
      actionItems: [
        { 
          ...
          label: 'Edit',
          action: (e, rowData) => navigate('/admin/brands/edit', { state: rowData }),
          permitted: permissionChecker('Brands', 'Update'),
        },
        { 
          icon: <Delete color='error'/>,
          label: 'Delete', 
          action: (e, rowData) => openDialog('Delete', false, rowData),
          permitted: permissionChecker('Brands', 'Delete'),
        },
        { 
          icon: <Lock color='warning'/>,
          label: 'Activate', 
          action: (e, rowData) => openDialog('Activate', false, rowData),
          permitted: permissionChecker('Brands', 'Activate'),
        },
        { 
          icon: <LockOpen color='warning'/>,
          label: 'Deactivate', 
          action: (e, rowData) => openDialog('Deactivate', false, rowData),
          permitted: permissionChecker('Brands', 'Deactivate'),
        },
    ...
    actionButtons: [
      {
        ...
        color: 'success',
        label: 'Export',
        onClick: () => exportToCSV(exportData, 'Brands'),
        skipPermission: true,
      },
      {
        icon: <Delete/>,
