...
const useCategories = () => {
  ...
  const onConfirmDialog = () => {
    if (dialogAction === 'Delete') {
      ...
      activateDeactivateCategory('activate');
    } else if (dialogAction === 'Deactivate') {
      activateDeactivateCategory('deactivate');
    }
  }
  ...
  const deleteCategory = async () => {
    ...
    setLoading(true);
    await axios.post('/categories/delete', apiData)
      .then(({data}) => {
        selected.map((data) => {
  ...
  const activateDeactivateCategory = async (action) => {
    const apiData = {
      ids: selected,
    };

    setLoading(true);
    await axios.post(`/categories/${action}`, apiData)
      .then(({data}) => {
        selected.map((data) => {
          let indexes = response.data.findIndex((i) => i.id === data)
    ...
    (permissionChecker('Categories', 'Update') 
      ...
      actionItems: [
        { 
          ...
          label: 'Edit',
          action: (e, rowData) => navigate('/admin/categories/edit', { state: rowData }),
          permitted: permissionChecker('Categories', 'Update'),
        },
        { 
          icon: <Delete color='error'/>,
          label: 'Delete', 
          action: (e, rowData) => openDialog('Delete', false, rowData),
          permitted: permissionChecker('Categories', 'Delete'),
        },
        { 
          icon: <Lock color='warning'/>,
          label: 'Activate', 
          action: (e, rowData) => openDialog('Activate', false, rowData),
          permitted: permissionChecker('Categories', 'Activate'),
        },
        { 
          icon: <LockOpen color='warning'/>,
          label: 'Deactivate', 
          action: (e, rowData) => openDialog('Deactivate', false, rowData),
          permitted: permissionChecker('Categories', 'Deactivate'),
        },
