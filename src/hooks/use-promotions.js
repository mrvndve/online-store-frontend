...
const usePromotions = () => {
  ...
  const onConfirmDialog = () => {
    if (dialogAction === 'Delete') {
      ...
      activateDeactivatePromotions('activate');
    } else if (dialogAction === 'Deactivate') {
      activateDeactivatePromotions('deactivate');
    }
  }
  ...
  const deletePromotion = async () => {
    ...
    setLoading(true);
    await axios.post('/promotions/delete', apiData)
      .then(({data}) => {
        selected.map((data) => {
  ...
  const activateDeactivatePromotions = async (action) => {
    const apiData = {
      ids: selected,
    };

    setLoading(true);
    await axios.post(`/promotions/${action}`, apiData)
      .then(({data}) => {
        selected.map((data) => {
          let indexes = response.data.findIndex((i) => i.id === data)
    ...
    (permissionChecker('Promotions', 'Update') 
      ...
      actionItems: [
        { 
          ...
          label: 'Edit',
          action: (e, rowData) => navigate('/admin/promotions/edit', { state: rowData }),
          permitted: permissionChecker('Promotions', 'Update'),
        },
        { 
          icon: <Delete color='error'/>,
          label: 'Delete', 
          action: (e, rowData) => openDialog('Delete', false, rowData),
          permitted: permissionChecker('Promotions', 'Delete'),
        },
        { 
          icon: <Lock color='warning'/>,
          label: 'Activate', 
          action: (e, rowData) => openDialog('Activate', false, rowData),
          permitted: permissionChecker('Promotions', 'Activate'),
        },
        { 
          icon: <LockOpen color='warning'/>,
          label: 'Deactivate', 
          action: (e, rowData) => openDialog('Deactivate', false, rowData),
          permitted: permissionChecker('Promotions', 'Deactivate'),
        },
    ...
    actionButtons: [
      {
        ...
        color: 'success',
        label: 'Export',
        onClick: () => exportToCSV(exportData, 'Promotions'),
        skipPermission: true,
      },
      {
        icon: <Delete/>,
