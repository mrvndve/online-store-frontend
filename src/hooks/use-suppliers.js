...
const useSuppliers = () => {
  ...
  const onConfirmDialog = () => {
    if (dialogAction === 'Delete') {
      ...
      activateDeactivateSupplier('activate');
    } else if (dialogAction === 'Deactivate') {
      activateDeactivateSupplier('deactivate');
    }
  }
  ...
  const deleteSupplier = async () => {
    ...
    setLoading(true);
    await axios.post('/suppliers/delete', apiData)
      .then(({data}) => {
        selected.map((data) => {
  ...
  const activateDeactivateSupplier = async (action) => {
    const apiData = {
      ids: selected,
    };

    setLoading(true);
    await axios.post(`/suppliers/${action}`, apiData)
      .then(({data}) => {
        selected.map((data) => {
          let indexes = response.data.findIndex((i) => i.id === data)
    ...
    (permissionChecker('Suppliers', 'Update') 
      ...
      actionItems: [
        { 
          ...
          label: 'Edit',
          action: (e, rowData) => navigate('/admin/suppliers/edit', { state: rowData }),
          permitted: permissionChecker('Suppliers', 'Update'),
        },
        { 
          icon: <Delete color='error'/>,
          label: 'Delete', 
          action: (e, rowData) => openDialog('Delete', false, rowData),
          permitted: permissionChecker('Suppliers', 'Delete'),
        },
        { 
          icon: <Lock color='warning'/>,
          label: 'Activate', 
          action: (e, rowData) => openDialog('Activate', false, rowData),
          permitted: permissionChecker('Suppliers', 'Activate'),
        },
        { 
          icon: <LockOpen color='warning'/>,
          label: 'Deactivate', 
          action: (e, rowData) => openDialog('Deactivate', false, rowData),
          permitted: permissionChecker('Suppliers', 'Deactivate'),
        },
    ...
    actionButtons: [
      {
        ...
        color: 'success',
        label: 'Export',
        onClick: () => exportToCSV(exportData, 'Suppliers'),
        skipPermission: true,
      },
      {
        icon: <Delete/>,
